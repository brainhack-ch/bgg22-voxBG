% made xmltree_hb a nested function within present function to read
% input; code was taken from within xmltree.m
% [07.5.2022] - HB

function this = read_gifti_file_hb(filename, this)
...
%==========================================================================
function tree = xmltree_hb(f)
fid = fopen(f,'rt');
if (fid == -1)
    error(['[XMLTree] Cannot open ' f]);
end
xmlstr = fread(fid,'*char')';
%xmlstr = fscanf(fid,'%c');
fclose(fid);
tree.tree = xml_parser_hb(xmlstr);
tree.filename = f;
